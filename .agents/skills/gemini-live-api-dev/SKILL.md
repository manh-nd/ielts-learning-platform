...
Use `gemini-3.8-live-extended-thinking` when your voice agent must evaluate complex data, plan multiple steps, or handle long-running tools. The model speaks natural conversational fillers (e.g. _"Checking flight options now..."_) while executing asynchronous tools in the background.
